...
onRecordCreateRequest((e) => {
  ...
  if (role !== "admin") {
    try {
      ...
      throw new BadRequestError("Las fechas coinciden con un período bloqueado.")
    } catch (error) {
  ...
  if (leaveType && leaveType.getBool("deductsBalance")) {
    ...
    try {
      ...
      if (requestedDays > available) {
        throw new BadRequestError("No hay saldo suficiente para esta ausencia.")
      }
    } catch (error) {
      if (error instanceof BadRequestError) throw error
    }
  ...
  try {
    ...
    throw new BadRequestError("Ya existe una solicitud que coincide con esas fechas.")
  } catch (error) {
    if (error instanceof BadRequestError) throw error
  }
      ...
      if (e.app.settings().smtp.enabled) {
        try {
          e.app.newMailClient().send(new MailerMessage({
            ...
            subject: "Nueva solicitud de ausencia en " + e.app.settings().meta.appName,
            text:
...
onRecordUpdateRequest((e) => {
  ...
  if (actorRole !== "admin" && actorRole !== "manager") {
    throw new ForbiddenError("No tienes permisos para resolver esta solicitud.")
  }
  if (
    original.getString("status") !== "pending" ||
...
onRecordAfterUpdateSuccess((e) => {
  ...
  try {
    ...
    notification.set("title", "Solicitud de ausencia " + (labels[status] || status))
    notification.set(
      "message",
    ...
    if (e.app.settings().smtp.enabled) {
      try {
        e.app.newMailClient().send(new MailerMessage({
          ...
          subject: "Tu solicitud de ausencia ha sido " + (labels[status] || status),
          text:
            "Hola " +
            employee.getString("name") +
...
onRecordCreateRequest((e) => {
  ...
  try {
    ...
    throw new BadRequestError("Ya hay una corrección pendiente para este fichaje.")
  } catch (error) {
    if (error instanceof BadRequestError) throw error
  }
